...
def _completed(stdout: str = "", stderr: str = "", returncode: int = 0) -> subprocess.CompletedProcess[bytes]:
...
@patch("scripts.lib.vault.subprocess.run")
def test_call_injects_vault_name_from_env(mock_run: MagicMock, monkeypatch: pytest.MonkeyPatch) -> None:
...
@patch("scripts.lib.vault.subprocess.run")
def test_call_omits_vault_when_env_unset(mock_run: MagicMock, monkeypatch: pytest.MonkeyPatch) -> None:
